...
class trials_master:
    ...
    def plot_states(self, s, std, dev, tau_std, tau_dev, run_rules, si_q_arr, si_q_dev_arr, mu_tones, run_obs, run_contexts, name, noise = True):
        ...
        ax.plot(dev, label='deviant', color='orange')

        for j in range(len(rules_long)):

            if rules_long[j] == 0:
                ax.axvspan(j - 0.5, j + 0.5, color='lightgrey', alpha=0.1)
            elif rules_long[j] == 1:
                ax.axvspan(j - 0.5, j + 0.5, color='darkgrey', alpha=0.1)
            elif rules_long[j] == 2:
                ax.axvspan(j - 0.5, j + 0.5, color='white', alpha=0.1)
        
        colors_tau = {
            tau_std[s][0]: 'rosybrown',
            tau_std[s][1]: 'lightcoral',
            tau_std[s][2]: 'indianred',
            tau_std[s][3]: 'brown'
        }
        ...
        if noise == True:
            ...
            ax.plot(indices_dev, obs_hz_array[indices_dev],color='darkorange', label='deviant observation', alpha=0.7,linestyle='None', marker='.', markersize=2)

        ax.set_xlabel('tone', fontsize = 20)
        ax.set_ylabel('state value', fontsize = 20)
        ...
        #ax.legend(handles=tau_legend_patches + handles, loc='lower right', fontsize = 14)
        plt.tight_layout()
        plt.savefig(f"trial_lists/sub-{self.config_H['participant_nr']}/plots/lgd_std_dev_session_{s+1}_{name}_noise_{noise}.png", dpi=300, bbox_inches='tight')
        #plt.show()
        plt.close()
    ...
    def generate_sessions(self):
        ...
        for s in range(0, self.config_H["n_sessions"]):
            ...
            self.plot_states(s, run_states_std, run_states_dev, tau_std, tau_dev, run_rules, si_q_arr, si_q_dev_arr, mu_tones, run_obs, run_contexts, name = 'input', noise = True)
            self.plot_states(s, run_states_std, run_states_dev, tau_std, tau_dev, run_rules, si_q_arr, si_q_dev_arr, mu_tones, run_obs, run_contexts, name = 'input', noise = False)
            self.plot_states(s, state_s_hz, state_d_hz, tau_std, tau_dev, run_rules, si_q_arr, si_q_dev_arr, mu_tones, obs_hz, run_contexts, name = 'hz', noise = True)
