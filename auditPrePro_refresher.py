...
# LOAD STIMULI
# ----------------------------------------------------------#
trials = pd.read_csv(f"{config['trial_dir']}sub-{participant}/sub-all_ses-{session}_trials.csv")
trials = trials[trials['run_n'] == run_info-1]
trials = trials.iloc[len(trials)//2:]
...
#---- insert catch trials
catch_all = np.zeros(len(pd.unique(trials["trial_n"])))
...
message.text = f"% korrekte Positionen in diesem Wiederholungsdurchgang: {np.abs(accuracy): .2f}\n\n\nBitte gib der Versuchsleitung Bescheid!"
message.color = (1, 1, 1)
message.draw()
win.flip()
