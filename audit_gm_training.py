...
class AuditGenerativeModel:
    ...
    def sample_observations(self, contexts, states):
        ...
        obs_noise_lvls = np.linspace(self.si_r, 0.1, n_tones)
        
        v = np.zeros(n_tones)
        cnt = -1

        for i in obs_noise_lvls:
            cnt += 1
            v[cnt] = self._sample_N_(0, obs_noise_lvls[cnt], 1)

        v = v.reshape((contexts.shape[0], contexts.shape[1]))
...
class NonHierachicalAuditGM(AuditGenerativeModel):
    ...
    def generate_run(
        ...
        # Sample observations
        obs, obs_noise_lvls = self.sample_observations(contexts, states)
        ...
        if return_pars:
            ...
            pars = (*pars, self.si_r)
            return contexts, states, obs, v, pars
        else:
            return contexts, states, obs, v
...
class HierarchicalAuditGM(AuditGenerativeModel):
    ...
    def generate_run(
        ...
        # Sample observations
        obs, v = self.sample_observations(contexts, states)
        ...
        if return_pi_rules:
            return_elements = (*return_elements, pi_rules, v)
