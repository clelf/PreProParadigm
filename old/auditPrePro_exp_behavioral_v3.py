...
for runn in np.unique(runs):
    ...
    positions = np.where(
    (np.array(runs) == runn) &
    np.isin(np.array(rule), [1, 2])
    )
    
    while done == False:
        catch_pos = [[]]*len(np.unique(runs))
        
        while len(catch_pos[runn]) < 10:
        ...
        all(catch_pos[runn][i] - catch_pos[runn][i-1] > 2 for i in range(1, len(catch_pos[runn])))
        and not np.isin(catch_pos[runn], [0, len(runs == runn)-1]).any()
        )

    data_run = np.array([runs == runn])
    catch_pos_run = np.zeros(len(data_run[0]))
    catch_pos[runn] = list(map(lambda x: x + 1, catch_pos[runn]))
    catch_pos_run[catch_pos[runn]] = 1
...
for i in range(0, int(n_trials) + 1):
    ...
    if i > 0 and (runs[i] != runs[i-1] or i == n_trials):
        ...
        accuracy_catch_run = len(is_catch[trial_run_slow[0]:trial_run_slow[1]]) == 1[0]/10
        ...
        message.text = f"% korrekt identifizierte Töne in diesem Durchgang: {accuracy_run: .2f}\n\n\n% korrekt identifizierte Regeln in diesem Durchgang: {accuracy_catch_run: .2f}\n\n\nKurze Pause!\n\n\nWeiter geht's mit der Leertaste"
        message.draw()
        win.flip()
        ...
        while ptb.GetSecs() - catch_start <= catch_duration:
            ...
            if catch_keys and not catch_press:
                ...
                if catch_name == key_pos_catch[0] and rule_correct == 0:
                    catch_correct = 1
                elif catch_name == key_pos_catch[0] and rule_correct == 0:
                    catch_correct = 1
                else:
                    catch_correct = 0
