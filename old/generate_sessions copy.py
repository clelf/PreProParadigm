...
class trials_master_new:
    ...
    def __init__(self):
        self.params = {
                ...
                "frequencies": [[ss.norm.rvs(0,1,1), None], [ss.norm.rvs(0,1,1), None]], # mu values for std 
                "taus": [16, 40, 160, 240], # taus in each session
                "dpos": [[2,3,4],[4,5,6],[0]], # deviant positions per rule
                "contexts": ['std', 'dev'], 
                "stat_std": 0.3, # stationary standard deviation of the processes
                "si_r": 0.15, # observation noise
                "d": -0.7,
    ...
    def get_mu_dev(self):
        ...
        for f in range(0,len(self.params["frequencies"])):
            ...
            sigma_total = self.params["stat_std"] + self.params["si_r"]
            mu_dev_comp = (self.params["d"]*sigma_total) + self.params["frequencies"][f][0]
