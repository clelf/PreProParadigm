...
def get_markov_sequence(diag, t_r3, n_trials, rule_init):
    ...
    # as discussed with Alex --> but: here theoretically some randomness could be added?s
    ii = np.array([
...
participant_nr = '04' # participant number
    ...
    for i in range(0, len(tau_std[s])): # for each tau create a sequence of rules
        ...
        for c in range(len(contexts)): 
            ...
            if c == 0:
                ...
                states[i][c][0] = np.array(ss.norm.rvs(mu_std_dev[c], si_q * tau_dev[s][i] / ((2 * tau_dev[s][i] - 1) ** 0.5))) # dev

            if t_tones == True:

                if dev_t_tones == True:
                    w = np.array(ss.norm.rvs(0, si_q, int(n_trials*8)-1))
                    proc_noise[i][c] = w
                    ...
                    if c == 0:
                        ...
                        w_dev = np.array(ss.norm.rvs(0, si_q, int(n_trials)-1))
    ...
    for i in range(4):
        ...
        for val in line_values:
            ...
            if ind_col == 0:
                ...
                stationary_std = si_q * tau_dev[s][i] / ((2 * tau_dev[s][i] - 1) ** 0.5)
